...
class StatsCollector:
    ...
    def get_deep_work_duration(self, start_date: date, end_date: date) -> int:
        ...
        deep_work_minutes = sum(
            ...
            for card in cards
            if card.productivity_score >= 80 and not self._is_unrecognized_card(card)
        )
    ...
    def get_category_distribution(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        for category, duration in sorted(
            category_duration.items(), key=lambda x: -x[1]
        ):
            result.append(
                {
                    "name": category,
    ...
    def get_hourly_efficiency(self, target_date: date) -> List[Dict]:
        ...
        # 初始化 24 小时数据
        hourly_data = {h: {"score_sum": 0.0, "duration": 0.0} for h in range(24)}
    ...
    def get_weekly_trend(self, end_date: date) -> List[Dict]:
        ...
        for i in range(6, -1, -1):  # 从7天前到今天
            ...
            for card in cards:
                ...
                if duration <= 0:
                    continue
                weighted_score += card.productivity_score * duration
                score_duration += duration

            if score_duration > 0:
                avg_score = weighted_score / score_duration
            else:
                avg_score = 0
    ...
    def get_activities(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        for card in cards:
            ...
            result.append(
                {
                    "start": card.start_time.strftime("%H:%M")
                    if card.start_time
                    else "",
                    "end": card.end_time.strftime("%H:%M") if card.end_time else "",
                    "date": card.start_time.strftime("%Y-%m-%d")
                    if card.start_time
                    else "",
                    "category": card.category or "其他",
                    "category_color": CATEGORY_COLORS.get(
                        card.category, CATEGORY_COLORS["其他"]
                    ),
                    "title": card.title,
