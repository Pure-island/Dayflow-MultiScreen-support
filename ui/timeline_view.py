...
class StatsSummaryWidget(QFrame):
    ...
    def _add_bar(self, category: str, minutes: float):
        ...
        # 进度条
        percentage = (
            (minutes / self._total_minutes * 100) if self._total_minutes > 0 else 0
        )
        bar = QProgressBar()
        bar.setRange(0, 100)
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.setStyleSheet(
                self.styleSheet().replace(
                    "background-color:", "transform: scale(0.98); background-color:"
                )
            )
...
class TimelineHeader(QWidget):
    ...
    def set_date(self, date: datetime):
        if self._current_date.date() == date.date():
    ...
    def _get_filtered_cards(self) -> List[ActivityCard]:
        ...
        for card in self._cards:
            ...
            if (
                ...
                or self._search_text in summary
