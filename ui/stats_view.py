...
class MetricCard(QFrame):
    ...
    def set_value(self, value: str, unit: str = ""):
        ...
        self.value_label.setText(
            f"{value}<span style='font-size: 14px; opacity: 0.7;'>{unit}</span>"
        )
        self._apply_style()
...
class MetricCardsRow(QWidget):
    ...
    def set_data(
        ...
        activity_count: int,
        prev_hours: float = 0,
        prev_efficiency: float = 0,
        prev_deep_work: int = 0,
        prev_activities: int = 0,
    ):
        """设置数据"""
        # 总时长
...
class DonutChart(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        for idx, (label, value, color) in enumerate(self._data):
            ...
            rect = QRectF(
                center.x() - r_outer, center.y() - r_outer, r_outer * 2, r_outer * 2
            )
            inner_rect = QRectF(
                center.x() - r_inner, center.y() - r_inner, r_inner * 2, r_inner * 2
            )
        ...
        text_rect = QRectF(
            center.x() - inner_radius, center.y() - 18, inner_radius * 2, 30
        )
        ...
        painter.setPen(QPen(QColor(t.text_muted)))
        subtext_rect = QRectF(
            center.x() - inner_radius, center.y() + 12, inner_radius * 2, 20
        )
...
class BarChartWidget(QWidget):
    ...
    def set_data(self, data: List[Dict], max_value: int = None):
        ...
        if max_value:
            ...
            max_total = (
                max(sum(d.get("categories", {}).values()) for d in data)
                if data
                else 480
            )
...
class LineChartWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i, (date, score) in enumerate(self._data):
            x = (
                margin_left + (chart_width * i / (point_count - 1))
                if point_count > 1
                else margin_left
            )
...
class GoalWidget(QWidget):
    ...
    def _update_display(self):
        ...
        # 进度条
        percent = (
            min(100, (self._current_hours / self._goal_hours) * 100)
            if self._goal_hours > 0
            else 0
        )
...
class WeekCompareWidget(QWidget):
    ...
    def _update_display(self):
        ...
        # 计算总时长
        this_total = sum(
            v for k, v in self._this_week_data.items() if not k.startswith("_")
        )
        last_total = sum(
            v for k, v in self._last_week_data.items() if not k.startswith("_")
        )
        ...
        self._add_summary_card(
            "本周总时长", this_total, last_total, "分钟", self.summary_layout
        )
        self._add_summary_card(
            "本周效率", this_score, last_score, "%", self.summary_layout
        )
        ...
        for cat in sorted(all_cats):
            ...
            diff_label.setStyleSheet(
                f"color: {diff_color}; font-size: 12px; font-weight: bold;"
            )
...
class AppUsageListWidget(QWidget):
    ...
    def apply_theme(self):
        ...
        self.empty_label.setStyleSheet(
            f"color: {t.text_muted}; font-size: 13px; padding: 8px 0;"
        )
        # 重新渲染行以应用主题色
        self._refresh()
...
class StatsPanel(QWidget):
    ...
    def _load_data(self):
        ...
        try:
            ...
            for i in range(days + days - 1, days - 1, -1):
                ...
                for card in cards:
                    if self._is_unrecognized_card(card):
                        continue
            ...
            for i in range(days - 1, -1, -1):
                ...
                for card in cards:
                    ...
                    if (
                        not self._is_unrecognized_card(card)
                        and card.productivity_score > 0
                    ):
                        total_score += card.productivity_score
                        score_count += 1
                    ...
                    if card.app_sites:
                        ...
                        for app, sec in zip(card.app_sites, normalized_seconds):
                            ...
                            app_usage_by_range[key] = (
                                app_usage_by_range.get(key, 0) + minutes_app
                            )

                bar_data.append({"date": date_str, "categories": categories})
            ...
            avg_efficiency = (
                total_score_range / score_count_range if score_count_range > 0 else 0
            )
            prev_hours = prev_total_minutes / 60
            prev_efficiency = (
                prev_total_score / prev_score_count if prev_score_count > 0 else 0
            )
            ...
            # 更新环形图
            donut_data = []
            for cat, minutes in sorted(
                category_minutes.items(), key=lambda x: x[1], reverse=True
            ):
            ...
            sorted_usage = sorted(
                app_usage_by_range.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def _is_unrecognized_card(self, card) -> bool:
