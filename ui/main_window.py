    ...
    def set_recording(self, recording: bool, paused: bool = False):
        ...
        if recording and not paused:
            ...
            self.status_label.setStyleSheet(
                f"color: {t.error}; font-size: 12px; font-weight: 600;"
            )
...
class SettingsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        api_desc = QLabel(
            "支持 OpenAI 兼容接口（心流API、OpenAI、DeepSeek、本地模型等）"
        )
    ...
    def _load_settings(self):
        ...
        self.email_receiver_input.setText(
            self.storage.get_setting("email_receiver", "")
        )
    ...
    def _test_connection(self):
        ...
        def run_test():
            provider = DayflowBackendProvider(
                api_base_url=api_url, api_key=api_key, model=api_model
            )
    ...
    def _export_data(self):
        ...
        try:
            ...
            with self.storage._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM timeline_cards ORDER BY start_time DESC"
                )
    ...
    def _send_test_email(self):
        ...
        self.email_result_label.setStyleSheet(
            "font-size: 13px; color: #9CA3AF; padding: 8px 0;"
        )
    ...
    def _show_email_success(self):
        ...
        self.email_result_label.setStyleSheet(
            f"font-size: 13px; color: {t.success}; padding: 4px 0;"
        )
    ...
    def _show_email_error(self, error: str):
        ...
        self.email_result_label.setStyleSheet(
            f"font-size: 13px; color: {t.error}; padding: 4px 0;"
        )
    ...
    def _on_test_email_result(self, success: bool):
        ...
        if success:
            self.email_result_label.setText("✅ 测试邮件发送成功！请检查收件箱")
            self.email_result_label.setStyleSheet(
                f"font-size: 13px; color: {t.success}; padding: 8px 0;"
            )
        else:
            self.email_result_label.setText("❌ 发送失败，请检查邮箱配置")
            self.email_result_label.setStyleSheet(
                f"font-size: 13px; color: {t.error}; padding: 8px 0;"
            )
    ...
    def _check_update(self):
        ...
        self.update_status_label.setStyleSheet(
            f"font-size: 13px; color: {t.text_secondary};"
        )
    ...
    @Slot(bool, str, str)
    def _on_check_update_result(
        self, has_update: bool, latest_version: str, release_notes: str
    ):
        """检查更新结果回调"""
        ...
        if has_update:
            ...
            self.update_status_label.setStyleSheet(
                f"font-size: 13px; color: {t.success}; font-weight: 600;"
            )
    ...
    def _on_download_complete(self, success: bool, error: str):
        ...
        if success:
            ...
            self.update_status_label.setStyleSheet(
                f"font-size: 13px; color: {t.error};"
            )
    ...
    def _install_update(self):
        ...
        if reply == QMessageBox.Yes:
            if self.update_manager.apply_update():
                ...
                QMessageBox.warning(
                    self, "安装失败", "无法启动更新程序，请手动下载安装最新版本。"
                )
...
class MainWindow(QMainWindow):
    ...
    def _do_auto_start_recording(self):
        ...
        try:
            ...
            self.tray_icon.showMessage(
                "Dayflow", "已自动开始录制 ✓", QSystemTrayIcon.Information, 2000
            )
            logger.info("自动录制已启动")
        except Exception as e:
        ...
        if self.recording_manager.is_recording:
            ...
            def stop_in_background():
                try:
                    ...
                    QMetaObject.invokeMethod(
                        self, "_on_recording_stopped", Qt.QueuedConnection
                    )
    ...
    def _on_recording_stopped(self):
        ...
        # 显示完成提示
        self.tray_icon.showMessage(
            "Dayflow", "录制已停止，数据已保存 ✓", QSystemTrayIcon.Information, 2000
        )
    ...
    def _open_github(self):
        ...
        webbrowser.open("https://github.com/Pure-island/Dayflow-MultiScreen-support")
    ...
    def _on_export_requested(self, date: datetime, cards: list):
        ...
        try:
            with open(file_path, "w", newline="", encoding="utf-8-sig") as f:
                ...
                for card in cards:
                    apps = (
                        ", ".join([app.name for app in card.app_sites])
                        if card.app_sites
                        else ""
                    )
                    writer.writerow(
                        [
                            card.start_time.strftime("%Y-%m-%d %H:%M:%S")
                            if card.start_time
                            else "",
                            card.end_time.strftime("%Y-%m-%d %H:%M:%S")
                            if card.end_time
                            else "",
    ...
    def _minimize_to_tray(self):
        ...
        self.hide()
        self.tray_icon.showMessage(
            "Dayflow", "应用已最小化到系统托盘", QSystemTrayIcon.Information, 2000
        )
    ...
    def _check_email_schedule(self):
        ...
        # 更新配置
        self.email_scheduler.email_service.config.sender_email = (
            self.storage.get_setting("email_sender", "")
        )
        self.email_scheduler.email_service.config.auth_code = self.storage.get_setting(
            "email_auth", ""
        )
        self.email_scheduler.email_service.config.receiver_email = (
            self.storage.get_setting("email_receiver", "")
        )
