...
| 🤖 **AI 智能分析** | OCR+LLM 或 视觉模型识别屏幕活动，自动归类 |
...
| ⚙️ **可配置参数** | 邮件时间、空闲暂停阈值、API 超时、重试次数等均可自定义 |
...
2. 配置 API 信息：
   ...
   - **API Key**：你的 API 密钥
   - **模型名称**：OCR+LLM 模式可用通用模型，VLM 模式需支持视觉
   - **分析模式**：OCR+LLM（推荐）/ VLM（图像）
   - **Thinking 模式**：开/关/低/中/高（不同服务商支持情况不同）
   - **空闲暂停**：无键鼠输入超过阈值自动暂停录制
   - **重试次数**：解析失败或结果为空时自动重试
3. 点击 **测试连接** 验证
4. 点击 **保存配置**
...
| 🖼️ **最小化传输** | VLM 模式仅发送关键帧图片；OCR+LLM 模式仅发送 OCR 文本 |
